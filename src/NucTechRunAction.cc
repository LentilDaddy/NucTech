...
void NucTechRunAction::EndOfRunAction(const G4Run* run)
{
  if (IsMaster())
  {
    ...
    const long long channelNO15 = NucTechSteppingAction::GetChannelNO15();
    const long long channelC12Alpha = NucTechSteppingAction::GetChannelC12Alpha();
    const long long channelPN15 = NucTechSteppingAction::GetChannelPN15();
    ...
    G4cout << "Photonuclear diagnostics:" << G4endl
           ...
           << "  Strict tagged reactions (n + 15O): " << taggedReactions << G4endl
           << "  Events with >=1 strict tag: " << eventsWithTaggedReaction << G4endl
           << "  Events with >=1 O15 track (any origin): " << eventsWithAnyF18 << G4endl
           << "  Events with photonuclear step AND any O15 track: " << eventsWithPhotonuclearAndAnyF18 << G4endl
           ...
           << "  Mean strict tags per primary: " << meanTaggedPerPrimary << G4endl
            ...
            printChannel("n + O15", channelNO15);
            printChannel("C12 + alpha", channelC12Alpha);
            printChannel("p + N15", channelPN15);
